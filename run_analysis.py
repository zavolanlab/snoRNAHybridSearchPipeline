...
def main(options):
    ...
    for f in glob.glob(options.input_dir + "/*.inputfasta"):
        ...
        search_anchors_script = 'scripts/rg_search_anchor_and_make_alignments.py'
    ...
    make_statistics_command = "python %s --input %s --output %s --length 15 --fpr 100.0 --dir %s" % (
                os.path.join(pip_dir, 'scripts/rg_make_stats_for_search.py'),
    ...
    for input_name, search_anchor_id in files_to_run.iteritems():
        ...
        convert_to_fasta_script = 'scripts/rg_convert_tab_to_fasta.py'
    ...
    for input_name, convert_mapped_to_bed_id in convert_to_bed_dependancies.iteritems():
        ...
        filter_bed_script = 'scripts/rg_filter_bed.py'
    ...
    for input_name, filter_bed_id in filter_bed_dependancies.iteritems():
        ...
        get_true_chrom_script = 'scripts/rg_get_true_chromosome_positions.py'
    ...
    for input_name, get_true_chrom_id in get_true_chrom_dependancies.iteritems():
        ...
        append_sequence_script = 'scripts/rg_extract_sequences.py'
    ...
    if settings["general"]['type'] == "CD":
        ...
        for input_name, append_sequence_id in append_sequence_dependancies.iteritems():
            ...
            append_score_script = 'scripts/rg_check_hybrids_with_plexy.py'
        ...
        for input_name, append_sequence_id in append_sequence_dependancies.iteritems():
            ...
            append_score_script = 'scripts/rg_check_hybrids_with_rnasnoop.py'
            append_score_command = """python {script} \\
                                            --input {input} \\
                                            --output {output} \\
                                            --snoRNA-paths {snorna_path} \\
    ...
    if settings["general"]['type'] == "CD":
        ...
        for input_name, append_plexy_id in append_score_dependancies.iteritems():
            ...
            append_duplex_script = 'scripts/rg_check_hybrids_with_rnaduplex.py'
